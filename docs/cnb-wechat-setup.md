...
写入：
- 仓库根 `.cnb.yml`（无需任何 tools/ 目录——上传走 Unity 内嵌的原生 .NET 实现，零外部依赖）
...
- `.cnb.yml` **不被** ignore
- 若用 Option B：`envs.yml` **不被** ignore（CI 要拉 secrets）
...
```bash
git add .cnb.yml ProjectSettings/T2FBuildSettings.asset
# Option B 额外：
git add envs.yml
...
需要扩展 `UploadWeChatFirstPackageStep`（或新增一个 step）把 tar 后的 `minigame.tar.gz` 也通过 `TencentCosUploader` 传到一个独立 prefix，例如 `wechat-artifacts/{version}/`。框架已经有原生上传链路，加这步只需要 ~30 行 C# 调用 uploader 的 directory mode。
