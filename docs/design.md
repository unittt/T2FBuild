...
└────────────────────┬────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  上传层（Unity 内嵌 .NET 实现，无外部依赖）                    │
│    ├── TencentCosClient (V5 签名 + HTTP PUT)                 │
│    └── TencentCosUploader → manifest mode / directory mode    │
│        → 腾讯云 COS（Bucket + CDN）                          │
└─────────────────────────────────────────────────────────────┘
```
...
└── Editor/
    ...
    │   └── Uploaders/
    │       └── TencentCos/              # 子 asmdef：V5 签名 + HTTP PUT 原生实现
    ...
    └── Window/
        ...
        └── BuildWindow.cs                # 开发期一键打包窗口
```

**为什么 Runtime 几乎为空**：框架只在 Editor 工作，玩家运行时不需要任何代码；AB 加载由业务侧的 `T2FResource` 处理，避免重复职责。

**为什么 Templates 用 `~` 后缀**：Unity 不会扫描带 `~` 后缀的目录，避免模板里的 yml 被当成资源处理。子目录按 CI 平台分组（`github/`、`cnb/`），`CITemplateInstallerWindow`（`Window > T2FBuild > CI Template Installer`）顶部有 CI 平台下拉，根据选择的平台扫描对应子目录列出可安装的模板，开发者勾选后一键复制到项目对应位置（GitHub → `.github/workflows/`，CNB → `<repo>/.cnb.yml`）。
...
- **`TencentCosClient`**：直接实现 COS V5 签名算法（HMAC-SHA1）+ HTTP PUT，约 150 行 C#；零外部依赖（仅用 .NET BCL）；本地和 CI 走同一份代码
- **`TencentCosUploader`**：包装 `TencentCosClient`，支持 manifest 模式（按文件列表上传）和 directory 模式（递归目录上传，用于 Player 静态站点）
- **`UploadAssetBundleStep` / `UploadPlayerStep` / `UploadWeChatFirstPackageStep`**：都在 Unity Editor 内同步运行，受 `T2FBUILD_UPLOAD_ENABLED` 环境变量门控
- **Credentials 注入**：BuildWindow 在调 BuildRunner 前自动从 `<project>/envs.yml` inject 4 个 env vars（`TENCENT_SECRET_ID` / `_KEY` / `COS_BUCKET` / `COS_REGION`）到进程，构建结束恢复；CI 通过 platform 原生 secret 机制注入（GitHub Secrets / CNB imports）
...
框架的 Unity 端 (`BuildEntry` CLI、原生 `TencentCosClient` HTTP 上传、AB/Uploader 抽象) 是 CI 平台无关的，只有 workflow yaml 因平台而异。模板按平台分目录，`CITemplateInstallerWindow` 让用户在 Project 内一键安装。
...
- 单 job 顺序：checkout → 计算参数 → 缓存 Library → `game-ci/unity-builder` 调 `BuildEntry.BuildWebGL`（`T2FBUILD_UPLOAD_ENABLED` 跟随 workflow input，开则 Unity 内嵌的原生 uploader 直接上传 AB + Player）→ upload-artifact → job summary 输出可访问 URL
- COS 直接作为 WebGL 静态托管：`TencentCosUploader` 按扩展名设置 `Content-Type` 和 `Content-Encoding`（`.br` → `br`, `.gz` → `gzip`），无需 COS 端额外配置
...
CNB 模板单文件包含 4 个触发块（webgl 手动 / wechat 手动 / `v*` 标签 / `wx-v*` 标签），Unity 命令与 GitHub 一致——都是 `BuildEntry.BuildXxx`，原生 .NET uploader 在 Unity 内部完成上传，仅外层 yaml 语法不同。Library 缓存通过 `volumes: ["/workspace/Library:copy-on-write"]` 实现。
...
打开 `Window > T2FBuild > CI Template Installer`，顶部选择 CI 平台（GitHub Actions 或 CNB），勾选需要的模板，点 Apply 自动写入到对应位置（GitHub → `<project>/.github/workflows/`，CNB → `<project>/.cnb.yml`），提交即可。Secrets 通过 `Edit > Project Settings > T2FBuild > Secrets` 编辑，落到 `<project>/envs.yml`（Option B）或单独的 secrets repo（Option A/C，需手改 `imports:` URL）。
...
| 4 | TencentCosUploader（原生 .NET V5 签名） | 验证上传链路，零外部依赖 |
